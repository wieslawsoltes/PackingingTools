...
public sealed class RpmFormatProvider : IPackageFormatProvider
{
    private readonly ILinuxProcessRunner _processRunner;
    private readonly ITelemetryChannel _telemetry;
    private readonly ILogger<RpmFormatProvider>? _logger;

    public RpmFormatProvider(ILinuxProcessRunner processRunner, ITelemetryChannel telemetry, ILogger<RpmFormatProvider>? logger = null)
    {
    ...
    public async Task<PackageFormatResult> PackageAsync(PackageFormatContext context, CancellationToken cancellationToken = default)
    {
        ...
        var outputPath = Path.Combine(context.Request.OutputDirectory, $"{context.Project.Name}.rpm");
        Directory.CreateDirectory(Path.GetDirectoryName(outputPath)!);

        var root = ResolvePackageRoot(context, "linux.rpm.root", issues);
        if (root is null)
        {
        ...
        var args = new List<string>
        {
            ...
            "--architecture",
            context.Project.Metadata.TryGetValue("linux.architecture", out var arch) ? arch : "x86_64",
            "--prefix",
            context.Project.Metadata.TryGetValue("linux.rpm.prefix", out var prefix) ? prefix : $"/opt/{context.Project.Name}"
        };

        if (context.Project.Metadata.TryGetValue("linux.rpm.description", out var description))
        {
            args.Add("--description");
            args.Add(description);
        }

        if (context.Project.Metadata.TryGetValue("linux.rpm.dependencies", out var dependencies))
        {
        ...
        if (!result.IsSuccess)
        {
            issues.Add(new PackagingIssue(
                "linux.rpm.fpm_failed",
                $"fpm failed with exit code {result.ExitCode}: {result.StandardError}",
                PackagingIssueSeverity.Error));
            return new PackageFormatResult(Array.Empty<PackagingArtifact>(), issues);
        }

        var architecture = context.Project.Metadata.TryGetValue("linux.architecture", out var archValue) ? archValue : "x86_64";
        ...
        if (context.Project.Metadata.TryGetValue("linux.rpm.description", out var descValue))
        {
